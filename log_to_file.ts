...
export const logToFile = async (
  ...
  if (process.env.NODE_ENV === "development") {
    ...
    const envLogsDirPath = "/Users/sacha/dev/standstill/standstill-bun/.logs";

    if (!envLogsDirPath) {
      console.warn(
        "Please define LOGS_PATH in your .env file to enable logging"
      );
      return;
    }
    ...
    const logsDirPath =
      dirPath ?? (subDir ? `${envLogsDirPath}/${subDir}` : envLogsDirPath);
